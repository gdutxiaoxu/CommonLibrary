...
/**
 * @author meitu.xujun  on 2017/7/31 15:15
 * @version 0.1
 */
...
public class ListUtils {
    ...
    public static <T> void copy(List<T> dest, List<T> src, boolean isClearDest) {
        if (ListUtils.isEmpty(src)) {
            return;
        }

        if (!ListUtils.isEmpty(dest)) {
            if(!isClearDest){
                throw new IllegalStateException("dest List not null");
            }else{
                dest.clear();
            }
